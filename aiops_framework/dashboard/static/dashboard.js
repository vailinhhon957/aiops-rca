...
function renderAnalysis(result) {
  ...
  if (anomaly.is_anomaly) {
    anomalyDecision.style.color = "#ae3f53";
  } else {
    anomalyDecision.style.color = "#18696e";
  }
...
function currentRecoveryContext() {
  ...
  return {
    ...
    context: {
      ...
      sample_name: latestAnalysis?.demo_context?.sample_name || sampleSelect.value,
...
async function executeRecovery(action) {
  ...
  try {
    ...
    recoveryLog.innerHTML = `
      ...
      <span>Status: ${result.status} | Severity: ${result.severity}</span><br/>
...
async function boot() {
  ...
  const [health, metadata, samples] = await Promise.all([
