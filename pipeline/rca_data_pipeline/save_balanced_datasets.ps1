...
if (-not $AnomalyMetadata) {
    $AnomalyMetadata = Join-Path $RepoRoot "pipeline\rca_data_pipeline\templates\balanced_manifests\anomaly_balanced_metadata.csv"
}
if (-not $RcaMetadata) {
    $RcaMetadata = Join-Path $RepoRoot "pipeline\rca_data_pipeline\templates\balanced_manifests\rca_balanced_metadata.csv"
}
if (-not $AnomalySnapshot) {
    $AnomalySnapshot = Join-Path $RepoRoot "data_anomaly_balanced"
}
if (-not $RcaSnapshot) {
    $RcaSnapshot = Join-Path $RepoRoot "data_rca_balanced"
}
...
Invoke-RebuildAndSnapshot -Label "anomaly balanced" -MetadataFile $AnomalyMetadata -SnapshotDir $AnomalySnapshot
Invoke-RebuildAndSnapshot -Label "RCA balanced" -MetadataFile $RcaMetadata -SnapshotDir $RcaSnapshot

Write-Host ""
Write-Host "Done." -ForegroundColor Green
Write-Host "Anomaly snapshot: $AnomalySnapshot"
Write-Host "RCA snapshot:     $RcaSnapshot"
Write-Host "Current working data/ now matches the last rebuild (RCA)."
