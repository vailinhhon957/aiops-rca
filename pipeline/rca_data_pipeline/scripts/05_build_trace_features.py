...
from pipeline.rca_data_pipeline.feature_engineering import build_trace_features
from pipeline.rca_data_pipeline.io_utils import latest_table, read_table, write_table


def main() -> None:
    parser = argparse.ArgumentParser(description="Build trace-level tabular features for anomaly detection.")
    ...
    for span_file in span_files:
        ...
        features_df = build_trace_features(spans_df, run_catalog_df=run_catalog_df)
    ...
    out_path = write_table(all_features, ANOMALY_ROOT / "trace_features")
    print(f"Trace features: {out_path}")
