...
if (-not $AnomalyMetadata) {
    $AnomalyMetadata = Join-Path $RepoRoot "pipeline\rca_data_pipeline\templates\balanced_manifests\v3\anomaly_balanced_v3_metadata.csv"
}
if (-not $RcaMetadata) {
    $RcaMetadata = Join-Path $RepoRoot "pipeline\rca_data_pipeline\templates\balanced_manifests\v3\rca_balanced_v3_metadata.csv"
}
if (-not $AnomalySnapshot) {
    $AnomalySnapshot = Join-Path $RepoRoot "data_anomaly_balanced_v3"
}
if (-not $RcaSnapshot) {
    $RcaSnapshot = Join-Path $RepoRoot "data_rca_balanced_v3"
}
...
Invoke-RebuildAndSnapshot -Label "anomaly balanced v3" -MetadataFile $AnomalyMetadata -SnapshotDir $AnomalySnapshot
Invoke-RebuildAndSnapshot -Label "RCA balanced v3" -MetadataFile $RcaMetadata -SnapshotDir $RcaSnapshot

Write-Host ""
Write-Host "Done." -ForegroundColor Green
Write-Host "Anomaly v3 snapshot: $AnomalySnapshot"
Write-Host "RCA v3 snapshot:     $RcaSnapshot"
Write-Host "Current working data/ now matches the last rebuild (RCA v3)."
